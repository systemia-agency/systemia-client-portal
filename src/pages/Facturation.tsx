...
export function Facturation() {
  ...
  const { subscription, invoices } = clientData

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Facturation</h1>
        <p className="text-muted-foreground text-sm mt-1">Gérez vos factures et votre abonnement.</p>
      </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="flex items-center gap-1.5 mt-1 text-xs text-muted-foreground">
                ...
                Prochain paiement : {subscription.nextBilling ? new Date(subscription.nextBilling).toLocaleDateString('fr-FR') : '—'}
              </div>
            </div>
          </div>
        </CardContent>
          ...
          <div className="space-y-2">
            ...
            {invoices.map((invoice) => (
              <div key={invoice.id} className="grid grid-cols-5 gap-4 items-center px-4 py-3 rounded-lg bg-background border border-border hover:shadow-sm transition-shadow">
                <div className="flex items-center gap-2"><Receipt className="h-4 w-4 text-muted-foreground" /><span className="text-sm font-medium text-foreground">{invoice.id}</span></div>
                <span className="text-sm text-muted-foreground">{new Date(invoice.date).toLocaleDateString('fr-FR')}</span>

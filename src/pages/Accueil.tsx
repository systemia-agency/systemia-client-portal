...
import { kpiData, leadsOverTime, projects } from '@/data/mock'
import { cn } from '@/lib/utils'

const kpis = [
  {
    label: 'Leads générés ce mois',
    value: kpiData.leadsThisMonth,
    trend: kpiData.leadsTrend,
    icon: Users,
    color: 'text-[var(--sys-blue)]',
    bg: 'bg-blue-50',
  },
  {
    label: 'Coût par lead',
    value: `${kpiData.costPerLead.toFixed(2)}€`,
    trend: kpiData.cplTrend,
    icon: Target,
    color: 'text-[var(--sys-violet)]',
    bg: 'bg-violet-50',
  },
  {
    label: 'Budget dépensé',
    value: `${kpiData.adSpend.toLocaleString('fr-FR')}€`,
    trend: kpiData.adSpendTrend,
    icon: DollarSign,
    color: 'text-[var(--sys-green)]',
    bg: 'bg-emerald-50',
  },
  {
    label: 'Campagnes actives',
    value: kpiData.activeCampaigns,
    trend: kpiData.campaignsTrend,
    icon: Megaphone,
...
export function Accueil() {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">Bonjour, Mon Entreprise</h1>
        <p className="text-muted-foreground text-sm mt-1">
          Voici un aperçu de vos performances marketing.
        </p>
      </div>
                ...
                return (
                  <div key={project.id} className="space-y-2">
                    ...
                    <div className="w-full bg-secondary rounded-full h-1.5">
                      <div
                        className="h-1.5 rounded-full bg-gradient-to-r from-[var(--sys-blue)] to-[var(--sys-violet)]"
